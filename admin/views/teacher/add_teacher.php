...
if (isset($_POST['btn-std'])){
    ...
    if(isset($name) && !empty($name) && isset($email) && !empty($email) && isset($password_encrypt) && !empty($password_encrypt) && isset($department) && !empty($department)){
        $check_std="SELECT * FROM `teachers` WHERE email= '$email'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            ///////
            /// INSERT TEACHER
         $insert_std = "INSERT INTO teachers (`name`, `email`, `password`, `department`,`status`, `reg_no`)
            VALUES ('$name', '$email', '$password_encrypt', '$department', '1', '$registration')";

            if (mysqli_query($conn, $insert_std)) {
                $msg='<div class="alert alert-success">Teacher is Added Successfully!</div>';
            } else {
                ...
                <div class="card m-b-30">
                    <div class="card-body">
                        ...
                        <form action="" method="post">
                            <h4 class="mt-0 header-title">Add New Teacher</h4>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" name="registration" placeholder="Id Number" class="form-control pd-r-80">
                                    <?= @$registration_msg ?>
                                </div>
                            </div>
                            <div class="form-group row">
