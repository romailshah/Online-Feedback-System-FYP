...
if (isset($_POST['btn-std'])){
    ...
    if(isset($name) && !empty($name) && isset($email) && !empty($email)){
        if(!empty($_POST['password']) && !empty($_POST['retype_password']) ) {
            ///////
            /// UPDATE STUDENT
            echo $update_std = "UPDATE admin SET `name`='$name',`email`='$email',`password`='$password_encrypt' WHERE id= '$id'";
            ...
            echo $update_std = "UPDATE admin SET `name`='$name',`email`='$email' WHERE id= '$id'";
                            ...
                            <div class="card-body">
                                ...
                                <form action="" method="post">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" name="email" value="<?= $row['email'] ?>" placeholder="Email" class="form-control pd-r-80" required="">
                                            <?= @$email_msg ?>
                                        </div>
                                    </div>
                                    <div class="form-group row">
